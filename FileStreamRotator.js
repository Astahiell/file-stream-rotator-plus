...
FileStreamRotator.getStream = function (options) {
    ...
    if(filename.match(/%DATE%/)){
        filename = filename.replace(/%DATE%/g,(curDate?curDate:self.getDate(null,dateFormat, options.utc)));
    }
    if(filename.match(/%YEAR%/)){
        filename = filename.replace(/%YEAR%/g,(self.getDate(null,"YYYY", options.utc)));
    }
    if (filename.match(/%MONTH%/)){
        filename = filename.replace(/%MONTH%/g,(self.getDate(null,"MM", options.utc)));
    }
    if(filename.match(/%DAY%/)){
        filename = filename.replace(/%DAY%/g,(self.getDate(null,"DD", options.utc)));
    }
    if(filename.match(/%HOUR%/)){
        filename = filename.replace(/%HOUR%/g,(self.getDate(null,"HH", options.utc)));
    }
    if(filename.match(/%MINUTE%/)){
        filename = filename.replace(/%MINUTE%/g,(self.getDate(null,"mm", options.utc)));
    }
    logfile = filename
